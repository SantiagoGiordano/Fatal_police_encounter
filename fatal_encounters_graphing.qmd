---
title: "Mapping Fatal Police Encounters in the US"
...
encs_by_state <- encs_by_state[-c(1), ] 
...
# Join data for number of fatal encounters by state data with geospatial data of US state
...
#### Packages used: 
```{r class.source = 'fold-show', code-fold : FALSE}
library(pacman)
p_load(dplyr, magrittr, broom, ggplot2, sf, tidyverse, maps, mapdata, 
       hrbrthemes, ggthemes, rnaturalearth, urbnmapr)
```
...
# Load Data
  # Fatal encounters data
fatal_encounters <- read.csv(
  here::here("Data", "Fatal_Encounters.nosync.csv"))
  # Geospatial data for US states
usa_states <- get_urbn_map("states", sf = TRUE)
```
...
# Summarize number of fatal encounters by state
encs_by_state <- fatal_encounters %>% 
                      group_by(State) %>% 
                      summarise(n_fatalencs = n())
# Get rid of first row
encs_by_state <- encs_by_state[-c(1), ] 
...
encs_by_state %<>% mutate(
    fatalencs_per_100k = (n_fatalencs / state_pop) * 100000)

# Print dataset created
head(encs_by_state, 5)
...
|   Finally, I merge this aggregated data with the geospatial data for US states.

```{r merge-data-2, warning=FALSE, message=FALSE}
# Set up final dataframe for plotting
